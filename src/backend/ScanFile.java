...
public class ScanFile implements Runnable {
    ...
    public static String[] asm_dir = { ".abort", ".align", ".ascii", ".asciz", ".balign", ".byte", ".comm", ".data",
            ".def", ".dim", ".double", ".eject", ".else", ".end", ".elseif", ".endef", ".endfunc", ".endif", ".endm",
            ".endr", ".equ", ".equiv", ".err", ".even", ".exitm", ".extern", ".fail", ".file", ".fill", ".float",
            ".func", ".global", ".hword", ".ident", ".if", ".include", ".incbin", ".int", ".irp", ".irpc", ".lcomm",
            ".lflags", ".line", ".ln", ".list", ".long", ".macro", ".mri", ".nolist", ".octa", ".org", ".p2align",
            // ".print",
            ".psize", ".purgem", ".quad", ".rept", ".sbttl", ".scl", ".section", ".set", ".short", ".single", ".size",
            ".sleb128", ".skip", ".space", ".stabd", ".stabn", ".stabs", ".string", ".struct", ".tag", ".text",
    ...
    public void scanStart() {
        ...
        /*
         * try { //for binary file outbin = new BufferedWriter(new FileWriter(name)); }
        ...
        try {
            ...
            if (temflag == 1) {
                ...
                while (a_end.size() != 0 && curent_line < a_end.get(val_bp) - 1) {
                    ...
                    // :"+frontend.FrontEnd.breakpointsEnabled);
                    ...
                    if ((strLine.startsWith(".read ") || strLine.startsWith(".READ "))) {
                        strLine = strLine.trim();
                        if (!(strLine.startsWith(".read") || strLine.startsWith(".READ "))) {
                            frontend.FrontEnd.statuswindow
                                    .append("ERROR in line " + ScanFile.curent_line + ": Unknown operation \n");
                        ...
                        if (FrontEnd.bflag == 0) {
                            ...
                            frontend.FrontEnd.statuswindow
                                    .setCaretPosition(frontend.FrontEnd.statuswindow.getText().length());
                        }
                     ...
                     * 
                     * try { //breakpoint reached /* exit from the while loop and start again when
                     * play pressed
                     *
                     * frontend.FrontEnd.activepane.getHighlighter().addHighlight(ini_pos,pos.
                     * intValue(), frontend.FrontEnd.cyanPainter);
                     * frontend.FrontEnd.bkpt_note.setVisible(true);
                     * frontend.FrontEnd.jButton12.setText("Run Again"); } catch
            ...
            frontend.FrontEnd.statuswindow.append("ERROR in line " + ScanFile.curent_line
                    + ": Unable to access the source file:" + path.getName() + " \n");
    ...
    static boolean checkForBarrelShift(String strLine) {
        ...
        for (int i = 0; i < shifts.length; i++) {
            if (strLine.contains(shifts[i])) {
                ...
                String imm = strLine.substring(id + 1);
                // System.out.println(imm.length());
                // not necessary that the entire line proceeding # is immediate
                int tmp = imm.length();
                while (tmp > 0 && !Character.isDigit(imm.charAt(tmp - 1))) {
                    tmp--;
                }
                // System.out.println("tmp = "+tmp);
                String appendAtlast = imm.substring(tmp);
                imm = imm.substring(0, tmp);
                // System.out.println( "********"+imm+"*******"+appendAtlast);
                String sub = "";
                ...
                // System.out.println("*** "+data);
                corectn_pt = corectn_pt.concat("#").concat(String.valueOf(data)).concat(appendAtlast);
                // System.out.println("AFTER SHIFT APPEND " + corectn_pt + " :))");
        ...
        if (!strLine.contains("[") && !strLine.contains("{")) {
            ...
            if (!strLine.contains("]")) {
                if (handlers.cmd_var == 1) {
                    ...
                    frontend.FrontEnd.statuswindow
                            .append("ERROR in line " + ScanFile.curent_line + ": Unknown syntax \n");
                    frontend.FrontEnd.statuswindow.setCaretPosition(frontend.FrontEnd.statuswindow.getText().length());
                }
                frontend.FrontEnd.exceptionraised++;
                return;
    ...
    void scanStart_cmd() throws IOException {
        ...
        // System.out.println(br);
        while ((strLine = br.readLine()) != null) {
            // System.out.println("****1 " + pos.intValue());
            ...
            if (FrontEnd.build_flag == 0) {
                backend.Register.r[15].b = pos.intValue();// update the prgram counter
                // System.out.println("***********4 " + pos.intValue());
            }
            ...
            if (strLine.contains("read") || strLine.contains("READ")) {
                ...
                /*
                 * ob=new ihandling(strLine); try { ob.t.join(); //
                 * Thread.currentThread().sleep(10000); // System.err.print("maine w8 kri"); }
