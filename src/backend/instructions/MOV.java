...
public class MOV implements InsInterface3 {
    ...
    public void execute(String ins, String token3, String token4, int x) {
        ...
        switch (token1) {
            ...
            case MOV:
                ...
                if (token4.startsWith("#")) {
                ...
                else if (backend.Register.convertRegister(token3) <= 15
                        && (backend.Register.convertRegister(token4) <= 15)) {
                    ...
                    if (FrontEnd.build_flag == 0)
                        if (backend.Register.convertRegister(token3) <= 7
                                && (backend.Register.convertRegister(token4) <= 7)) {
    ...
    public static void execMOV(String token3, String token4, int x, int y) {
        ...
        if (no2 > -1) {
            ...
            if (no3 > -1) {
                ...
                if (handlers.cmd_var == 1) {
                    System.out.println("\nERROR in line " + backend.ScanFile.curent_line + ":" + token3
                            + " is not a valid operand.");
                } else {

                    frontend.FrontEnd.statuswindow.append("\nERROR in line " + backend.ScanFile.curent_line + ":"
                            + token3 + " is not a valid operand.");
        ...
        if (no2 > -1) {
            ...
            if (token4.startsWith("#")) {
                ...
                try {
                    ...
                    if (handlers.cmd_var == 1) {
                        System.out.println("\nERROR in line " + backend.ScanFile.curent_line + ":" + token4
                                + " is not a valid operand.");
                    } else {

                        frontend.FrontEnd.statuswindow.append("\nERROR in line " + backend.ScanFile.curent_line + ":"
                                + token4 + " is not a valid operand.");
                        frontend.FrontEnd.statuswindow
                                .setCaretPosition(frontend.FrontEnd.statuswindow.getText().length());
                    }
                    frontend.FrontEnd.exceptionraised++;
    ...
    public static void execMOV(String token3, String token4) {
        //System.out.println("MOV " + token3 + " " + token4);
        String num;
                    ...
                    if (no3 == 14) {
                        ...
                        if (backend.Register.r[no3].b != backend.ScanFile.endOfProgram
                            ...
                            try {
                                ...
                                if (handlers.cmd_var == 1) {
                                    System.out.println("\nERROR in line " + backend.ScanFile.curent_line + ":"
                                            + "MOV operation failed");
                                } else {

                                    frontend.FrontEnd.statuswindow.append("\nERROR in line "
                                            + backend.ScanFile.curent_line + ": MOV operation failed.");
                                    frontend.FrontEnd.statuswindow
                                            .setCaretPosition(frontend.FrontEnd.statuswindow.getText().length());
                                }
                                frontend.FrontEnd.exceptionraised++;
         ...
         * 
         * backend.ScanFile.outbin.append((char)(binaryconvert[i]+'0')); } catch
